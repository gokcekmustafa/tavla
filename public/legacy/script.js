...
function syncCheckerSizeToBoard() {
  if (window.__designActive) return;
...
function applyDesignCheckerSizes() {
  if (!window.__designActive || !designData) return;
  var p = designActiveProfile || detectDesignProfile();
  var s = designData[p] || DESIGN_DEFAULTS[p];
  if (!s) return;
  document.querySelectorAll('.checker').forEach(function(el) {
    el.style.width = s.checkerSize + 'px';
    el.style.height = s.checkerSize + 'px';
  });
}
...
// ── Design Mode ──────────────────────────────────────
window.__designActive = false;

var DESIGN_PROFILES = ['desktop', 'portrait', 'landscape'];
var DESIGN_PROFILE_LABELS = { desktop:'Masaüstü', portrait:'Dikey', landscape:'Yatay' };

var DESIGN_DEFAULTS = {
  desktop:  { checkerSize:36, pointLaneWidth:91, pointLaneHeight:90, offAreaWidth:30, barPaddingV:6, hudCardPadding:8, shellPadding:8, boardHeightRatio:390 },
  portrait: { checkerSize:20, pointLaneWidth:91, pointLaneHeight:90, offAreaWidth:26, barPaddingV:5, hudCardPadding:6, shellPadding:6, boardHeightRatio:440 },
  landscape:{ checkerSize:16, pointLaneWidth:91, pointLaneHeight:90, offAreaWidth:18, barPaddingV:3, hudCardPadding:3, shellPadding:3, boardHeightRatio:340 },
};

var DESIGN_SLIDERS = [
  { key:'checkerSize',     label:'Taş Boyutu',       min:12, max:60, unit:'px' },
  { key:'pointLaneWidth',  label:'Üçgen Genişliği',  min:50, max:100, unit:'%' },
  { key:'pointLaneHeight', label:'Üçgen Yüksekliği', min:50, max:100, unit:'%' },
  { key:'offAreaWidth',    label:'Toplama Alanı',     min:12, max:60, unit:'px' },
  { key:'barPaddingV',     label:'Bar Alanı',         min:0,  max:20, unit:'px' },
  { key:'hudCardPadding',  label:'HUD Sıkışıklığı',  min:2,  max:20, unit:'px' },
  { key:'shellPadding',    label:'Kenar Boşluğu',    min:0,  max:30, unit:'px' },
  { key:'boardHeightRatio',label:'Board Yükseklik',  min:280,max:580, unit:'' },
];

var designData = null;
var designPanel = null;
var designActiveProfile = 'desktop';

function detectDesignProfile() {
  return window.innerWidth >= 1025 ? 'desktop'
...
function loadDesignSettings() {
  try {
    var saved = localStorage.getItem('tavla-design-v2');
    if (saved) {
      var parsed = JSON.parse(saved) || {};
      designData = {};
      DESIGN_PROFILES.forEach(function(p) {
        designData[p] = {};
        var def = DESIGN_DEFAULTS[p];
        var sv = parsed[p] || {};
        for (var k in def) designData[p][k] = sv.hasOwnProperty(k) ? Number(sv[k]) : def[k];
      });
      return;
    }
  } catch(e) {}
...
function saveDesignSettings() {
  if (!designData) {
    ...
    DESIGN_PROFILES.forEach(function(p) {
      designData[p] = {};
      for (var k in DESIGN_DEFAULTS[p]) designData[p][k] = DESIGN_DEFAULTS[p][k];
    });
  }
  DESIGN_PROFILES.forEach(function(p) {
    if (!designData[p]) designData[p] = {};
    for (var k in DESIGN_DEFAULTS[p]) {
      if (designData[p][k] === undefined || designData[p][k] === null) designData[p][k] = DESIGN_DEFAULTS[p][k];
    }
  });
  try { localStorage.setItem('tavla-design-v2', JSON.stringify({ desktop:designData.desktop, portrait:designData.portrait, landscape:designData.landscape })); } catch(e) {}
  window.__designActive = true;
}

function applyProfile(profile) {
  var s = designData ? designData[profile] : DESIGN_DEFAULTS[profile];
  if (!s) return;
  document.body.style.setProperty('--checker-size', s.checkerSize + 'px');
  document.querySelectorAll('.checker').forEach(function(el) {
    el.style.width = s.checkerSize + 'px';
    el.style.height = s.checkerSize + 'px';
  });
  document.documentElement.style.setProperty('--point-lane-width', s.pointLaneWidth + '%');
  document.documentElement.style.setProperty('--point-lane-height', s.pointLaneHeight + '%');
  document.querySelectorAll('.off-area').forEach(function(el) { el.style.width = s.offAreaWidth + 'px'; });
  var bz = document.querySelector('.bar-zone'); if (bz) bz.style.padding = s.barPaddingV + 'px 2px';
  document.querySelectorAll('.turn-card, .status-card, .controls-card').forEach(function(el) { el.style.padding = s.hudCardPadding + 'px'; });
  var sh = document.querySelector('.app-shell');
  if (sh) { sh.style.paddingTop = sh.style.paddingLeft = sh.style.paddingRight = s.shellPadding + 'px'; sh.style.paddingBottom = (s.shellPadding + 64) + 'px'; }
  var bg = document.querySelector('.board-grid'); if (bg) bg.style.aspectRatio = '572 / ' + s.boardHeightRatio;
  window.__designActive = true;
}

function createDesignUI() {
  var btn = document.createElement('button');
  btn.className = 'design-toggle-btn';
  btn.textContent = 'Tasar\u0131m';
  btn.addEventListener('click', toggleDesignPanel);
  document.body.appendChild(btn);

  var panel = document.createElement('aside');
  panel.className = 'design-panel';
  panel.innerHTML =
    '<div class="design-panel-header"><span>Tasar\u0131m Ayarlar\u0131</span><button class="design-close-btn">&times;</button></div>' +
    '<div class="design-tabs"></div>' +
    '<div class="design-panel-body"></div>' +
    '<div class="design-panel-footer"><button class="design-save-btn">Kaydet</button><button class="design-reset-btn">S\u0131f\u0131rla</button></div>';
  document.body.appendChild(panel);

  var tabsEl = panel.querySelector('.design-tabs');
  DESIGN_PROFILES.forEach(function(p) {
    var tb = document.createElement('button');
    tb.className = 'design-tab' + (p === designActiveProfile ? ' active' : '');
    tb.textContent = DESIGN_PROFILE_LABELS[p];
    tb.dataset.profile = p;
    tb.addEventListener('click', function() { switchDesignTab(p); });
    tabsEl.appendChild(tb);
  });

  var bodyEl = panel.querySelector('.design-panel-body');
  DESIGN_SLIDERS.forEach(function(s) {
    var row = document.createElement('div');
    row.className = 'design-slider-row';
    var label = document.createElement('label'); label.textContent = s.label;
    var val = document.createElement('span'); val.className = 'design-slider-val';
    var input = document.createElement('input'); input.type = 'range'; input.min = s.min; input.max = s.max; input.dataset.key = s.key;
    input.addEventListener('input', function() { onDesignSliderInput(s.key); });
    row.append(label, val, input);
    bodyEl.appendChild(row);
  });

  panel.querySelector('.design-close-btn').addEventListener('click', toggleDesignPanel);
  panel.querySelector('.design-save-btn').addEventListener('click', function() {
    readSliderValues();
    saveDesignSettings();
    showDesignToast('Ayarlar kaydedildi');
    toggleDesignPanel();
  ...
  panel.querySelector('.design-reset-btn').addEventListener('click', function() {
    DESIGN_PROFILES.forEach(function(p) {
      if (designData) for (var k in DESIGN_DEFAULTS[p]) designData[p][k] = DESIGN_DEFAULTS[p][k];
    });
    syncSliderUI();
    applyProfile(designActiveProfile);
    showDesignToast('S\u0131f\u0131rland\u0131');
  });
...
function initDesignDrag() {
  ...
  document.addEventListener('mousedown', function(e) {
    var h = e.target.closest('.design-panel-header');
    if (!h || !designPanel || !designPanel.classList.contains('open')) return;
    ds = { sx:e.clientX, sy:e.clientY, lx:designPanel.offsetLeft, ly:designPanel.offsetTop };
    e.preventDefault();
  });
  document.addEventListener('mousemove', function(e) {
    if (!ds) return;
    designPanel.style.left = (ds.lx + e.clientX - ds.sx) + 'px';
    designPanel.style.top  = (ds.ly + e.clientY - ds.sy) + 'px';
  ...
  document.addEventListener('touchstart', function(e) {
    ...
    if (!h || !designPanel || !designPanel.classList.contains('open')) return;
    var t = e.touches[0];
    ds = { sx:t.clientX, sy:t.clientY, lx:designPanel.offsetLeft, ly:designPanel.offsetTop };
  }, { passive:true });
  document.addEventListener('touchmove', function(e) {
    if (!ds) return;
    var t = e.touches[0];
    designPanel.style.left = (ds.lx + t.clientX - ds.sx) + 'px';
    designPanel.style.top  = (ds.ly + t.clientY - ds.sy) + 'px';
    designPanel.style.right = designPanel.style.bottom = 'auto';
  }, { passive:true });
  document.addEventListener('touchend', function() { ds = null; });
}

function switchDesignTab(profile) {
  readSliderValues();
  designActiveProfile = profile;
  var tabs = designPanel.querySelectorAll('.design-tab');
  for (var i = 0; i < tabs.length; i++) tabs[i].classList.toggle('active', tabs[i].dataset.profile === profile);
  syncSliderUI();
...
function readSliderValues() {
  ...
  var inputs = designPanel.querySelectorAll('input[type="range"]');
  for (var i = 0; i < inputs.length; i++) {
    designData[designActiveProfile][inputs[i].dataset.key] = Number(inputs[i].value);
  }
}

function getSliderUnit(key) {
  for (var i = 0; i < DESIGN_SLIDERS.length; i++) {
    if (DESIGN_SLIDERS[i].key === key) return DESIGN_SLIDERS[i].unit;
  }
  return '';
}

function syncSliderUI() {
  var data = designData ? designData[designActiveProfile] : DESIGN_DEFAULTS[designActiveProfile];
  var inputs = designPanel.querySelectorAll('input[type="range"]');
  for (var i = 0; i < inputs.length; i++) {
    var key = inputs[i].dataset.key;
    var val = data ? data[key] : DESIGN_DEFAULTS[designActiveProfile][key];
    if (val === undefined || val === null) val = DESIGN_DEFAULTS[designActiveProfile][key];
    inputs[i].value = val;
    var vs = inputs[i].closest('.design-slider-row').querySelector('.design-slider-val');
    vs.textContent = val + getSliderUnit(key);
  }
}

function onDesignSliderInput(key) {
  var inputs = designPanel.querySelectorAll('input[type="range"]');
  for (var i = 0; i < inputs.length; i++) {
    if (inputs[i].dataset.key === key) {
      ...
      designData[designActiveProfile][key] = val;
      applyProfile(designActiveProfile);
      break;
    }
  }
...
function toggleDesignPanel() {
  var open = designPanel.classList.toggle('open');
  if (open) {
    var btn = document.querySelector('.design-toggle-btn');
    if (btn) {
    ...
    designActiveProfile = detectDesignProfile();
    var tabs = designPanel.querySelectorAll('.design-tab');
    for (var i = 0; i < tabs.length; i++) tabs[i].classList.toggle('active', tabs[i].dataset.profile === designActiveProfile);
    syncSliderUI();
  }
}
...
function showDesignToast(msg) {
  ...
  t.className = 'design-toast';
  t.textContent = msg;
  document.body.appendChild(t);
  setTimeout(function() { t.remove(); }, 2200);
}

function applyDesignSettingsIfAny() {
  loadDesignSettings();
  if (designData) {
    window.__designActive = true;
    designActiveProfile = detectDesignProfile();
...
function onDesignViewportChange() {
  if (!window.__designActive || !designData) return;
  var p = detectDesignProfile();
  if (p !== designActiveProfile) {
    designActiveProfile = p;
    applyProfile(p);
  }
}

if (typeof window !== 'undefined') {
  var designResizeTimer = null;
  window.addEventListener('resize', function() {
    if (designResizeTimer) clearTimeout(designResizeTimer);
    designResizeTimer = setTimeout(onDesignViewportChange, 300);
  });
}

var designUrlParam = new URLSearchParams(window.location.search).get('design') === '1';
