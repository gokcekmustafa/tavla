...
import "../../App.css";
import "./UI101okey.css";
import "./okey101-mobile-landscape.css";
import "./UI101okey-portrait.css";
import "./UI101okey-desktop.css";
import {
...
type OkeyPrototypeLiveAction = {
  ...
  publicRackState?: OkeyPrototypeRackState;
  publicWallTiles?: OkeyPrototypeTile[];
  publicDiscardPile?: OkeyPrototypeDiscardEntry[];
  publicWinnerSeat?: OkeyPrototypeSeatNo | null;
  publicHandDrawn?: boolean;
  publicHandCompleted?: boolean;
  publicSeatPenaltyTotals?: Record<OkeyPrototypeSeatNo, number>;
  publicSeatHandWins?: Record<OkeyPrototypeSeatNo, number>;
  publicDrawHandCount?: number;
...
const OKEY_PROTOTYPE_SEAT_RESERVATION_SESSION_KEY = "tavla.okey.prototype.seat.reservation.v1";
...
const REALTIME_LOBBY_CHANNEL_PREFIX = "okey101-global-lobby-v2";
...
const SEAT_NULL_MERGE_GRACE_MS = 60_000;
...
const OKEY_PROTOTYPE_WIN_REWARD_POINTS = -101;
const OKEY_PROTOTYPE_JOKER_WIN_REWARD_POINTS = -202;
const OKEY_PROTOTYPE_JOKER_SINGLE_TURN_OPEN_WIN_REWARD_POINTS = -404;
const OKEY_PROTOTYPE_OPENED_JOKER_LEFT_PENALTY_POINTS = 101;
...
const OKEY_PROTOTYPE_RACK_SLOT_COLUMNS = 15;
...
const OKEY_PROTOTYPE_LOCAL_BOT_TABLE_ID_PREFIX = "local-bot-";
...
function pickOkeyPrototypeBotDiscardTileId(
  ...
  const pickSafe = (candidates: typeof sorted) => {
    for (const entry of candidates) {
      if (!avoidTileIds?.has(entry.tile.id)) return entry.tile.id;
    }
    return candidates[0]?.tile.id ?? null;
  };
  if (difficulty === "easy") {
    if (Math.random() < 0.55) {
      const safe = rack.filter((t) => !avoidTileIds?.has(t.id));
      if (safe.length > 0) return safe[Math.floor(Math.random() * safe.length)]?.id ?? null;
      return rack[Math.floor(Math.random() * rack.length)]?.id ?? null;
    }
    return pickSafe(sorted.slice(0, 4));
  }
  if (difficulty === "normal") {
    return pickSafe(sorted.slice(0, 2));
  }
  return pickSafe(sorted);
}
...
type OkeyPrototypePairOpenPlan = {
  groups: OkeyPrototypeTile[][];
  pairCount: number;
  jokerPairCount: number;
};

function isOkeyPrototypeExactPairMatch(left: OkeyPrototypeTile, right: OkeyPrototypeTile) {
  if (left.kind !== "normal" || right.kind !== "normal") return false;
...
function areOkeyPrototypeTilesAdjacentInSlotOrder(
  ...
  if (!orderedSlotTileIds || orderedSlotTileIds.length === 0 || tiles.length < 2) return false;
  ...
  const sortedIndexes = tiles
    .map((tile) => slotIndexByTileId.get(tile.id))
    .filter((index): index is number => typeof index === "number")
    .sort((left, right) => left - right);
  if (sortedIndexes.length !== tiles.length) return false;
  for (let index = 1; index < sortedIndexes.length; index += 1) {
    if (sortedIndexes[index] !== sortedIndexes[index - 1] + 1) return false;
  }
  return true;
}
...
function buildOkeyPrototypePairOpenPlan(
  ...
  const tileById = new Map<string, OkeyPrototypeTile>();
  tiles.forEach((tile) => {
    tileById.set(tile.id, tile);
  });
  const usedTileIds = new Set<string>();
  const groups: OkeyPrototypeTile[][] = [];
  let jokerPairCount = 0;

  const addPair = (left: OkeyPrototypeTile, right: OkeyPrototypeTile) => {
    if (left.id === right.id) return false;
    if (usedTileIds.has(left.id) || usedTileIds.has(right.id)) return false;
  ...
  if (orderedSlotTileIds && orderedSlotTileIds.length > 1) {
    for (let index = 0; index < orderedSlotTileIds.length - 1; index += 1) {
      const leftId = orderedSlotTileIds[index];
      const rightId = orderedSlotTileIds[index + 1];
      if (!leftId || !rightId) continue;
      const leftTile = tileById.get(leftId);
      const rightTile = tileById.get(rightId);
      if (!leftTile || !rightTile) continue;
      if (usedTileIds.has(leftTile.id) || usedTileIds.has(rightTile.id)) continue;
      tryAddPairFromAdjacentTiles(leftTile, rightTile);
    }
  }

  const collectRemainingTiles = (predicate: (tile: OkeyPrototypeTile) => boolean) => {
    const ordered: OkeyPrototypeTile[] = [];
    const seen = new Set<string>();
    const tryPushTile = (tile: OkeyPrototypeTile | undefined) => {
      if (!tile) return;
    ...
    if (orderedSlotTileIds) {
      orderedSlotTileIds.forEach((slotTileId) => {
        if (!slotTileId) return;
        tryPushTile(tileById.get(slotTileId));
      });
    }
    tiles.forEach((tile) => tryPushTile(tile));
    return ordered;
  };
  ...
  groupedByKey.forEach((bucket) => {
    const safeTiles = bucket.slice().sort((left, right) => left.id.localeCompare(right.id));
    while (safeTiles.length >= 2) {
      const left = safeTiles.shift();
      const right = safeTiles.shift();
      if (!left || !right) break;
      addPair(left, right);
    }
  });
  ...
  while (remainingNormalSingles.length > 0 && remainingJokers.length > 0) {
    ...
    const jokerTile = remainingJokers.shift();
    if (!normalTile || !jokerTile) break;
    addPair(normalTile, jokerTile);
  }

  while (remainingJokers.length >= 2) {
    const leftJoker = remainingJokers.shift();
    const rightJoker = remainingJokers.shift();
    if (!leftJoker || !rightJoker) break;
...
function normalizeSignedInt(value: unknown, fallback = 0) {
  const num = Number(value);
  if (!Number.isFinite(num)) return fallback;
...
function sanitizeOkeyPrototypeTableId(raw: unknown) {
  if (typeof raw !== "string") return "";
...
function normalizeOkeyPrototypeSeatSignedNumericState(raw: unknown): Record<OkeyPrototypeSeatNo, number> | null {
  if (!raw || typeof raw !== "object") return null;
  const candidate = raw as Partial<Record<OkeyPrototypeSeatNo, unknown>>;
  const next = createDefaultOkeyPrototypeSeatWinState();
  OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
    next[seatNo] = normalizeSignedInt(candidate[seatNo], 0);
  });
  return next;
}

function normalizeOkeyPrototypeSeatNonNegativeNumericState(raw: unknown): Record<OkeyPrototypeSeatNo, number> | null {
  if (!raw || typeof raw !== "object") return null;
  const candidate = raw as Partial<Record<OkeyPrototypeSeatNo, unknown>>;
  const next = createDefaultOkeyPrototypeSeatWinState();
  OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
    next[seatNo] = Math.max(0, normalizeNonNegativeInt(candidate[seatNo], 0));
  });
  return next;
}

function sanitizeOkeyPrototypeLastHandSummary(raw: unknown) {
  if (typeof raw !== "string") return "";
  return raw.trim().replace(/\s+/g, " ").slice(0, 220);
}

function sanitizeOkeyPrototypeOutcomeKey(raw: unknown) {
  if (typeof raw !== "string") return "";
...
function normalizeOkeyPrototypeRackState(raw: unknown): OkeyPrototypeRackState | null {
  if (!raw || typeof raw !== "object") return null;
  const candidate = raw as Partial<Record<OkeyPrototypeSeatNo, unknown>>;
  const next: OkeyPrototypeRackState = {
    1: [],
    2: [],
    3: [],
    4: [],
  ...
  OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
    const row = Array.isArray(candidate[seatNo]) ? candidate[seatNo] : [];
    next[seatNo] = row
      .map((tileRaw) => normalizeOkeyPrototypeTile(tileRaw))
      .filter((tile): tile is OkeyPrototypeTile => Boolean(tile))
      .slice(0, OKEY_PROTOTYPE_RACK_SLOT_COUNT);
  });
  return next;
}

function normalizeOkeyPrototypeTileList(raw: unknown, limit = 106): OkeyPrototypeTile[] | null {
...
function normalizeOkeyPrototypeDiscardList(raw: unknown, limit = 40): OkeyPrototypeDiscardEntry[] | null {
  if (!Array.isArray(raw)) return null;
  return raw
    .map((entryRaw) => normalizeOkeyPrototypeDiscardEntry(entryRaw))
    .filter((entry): entry is OkeyPrototypeDiscardEntry => Boolean(entry))
    .slice(0, limit);
}
...
function normalizeOkeyPrototypeLiveAction(raw: unknown): OkeyPrototypeLiveAction | null {
  ...
  const publicRackState = normalizeOkeyPrototypeRackState(candidate.publicRackState);
  const publicWallTiles = normalizeOkeyPrototypeTileList(candidate.publicWallTiles);
  const publicDiscardPile = normalizeOkeyPrototypeDiscardList(candidate.publicDiscardPile, 40);
  const hasPublicWinnerSeat = Object.prototype.hasOwnProperty.call(candidate, "publicWinnerSeat");
  const publicWinnerSeatRaw = hasPublicWinnerSeat ? candidate.publicWinnerSeat : undefined;
  const normalizedPublicWinnerSeat = publicWinnerSeatRaw === null ? null : normalizeOkeyPrototypeSeatNo(publicWinnerSeatRaw);
  const publicWinnerSeat = hasPublicWinnerSeat ? (normalizedPublicWinnerSeat ?? null) : undefined;
  const hasPublicHandDrawn = Object.prototype.hasOwnProperty.call(candidate, "publicHandDrawn");
  const publicHandDrawn = hasPublicHandDrawn ? Boolean(candidate.publicHandDrawn) : undefined;
  const hasPublicHandCompleted = Object.prototype.hasOwnProperty.call(candidate, "publicHandCompleted");
  const publicHandCompleted = hasPublicHandCompleted ? Boolean(candidate.publicHandCompleted) : undefined;
  const publicSeatPenaltyTotals = normalizeOkeyPrototypeSeatSignedNumericState(candidate.publicSeatPenaltyTotals);
  const publicSeatHandWins = normalizeOkeyPrototypeSeatNonNegativeNumericState(candidate.publicSeatHandWins);
  const hasPublicDrawHandCount = Object.prototype.hasOwnProperty.call(candidate, "publicDrawHandCount");
  const publicDrawHandCount = hasPublicDrawHandCount
    ? Math.max(0, normalizeNonNegativeInt(candidate.publicDrawHandCount, 0))
    : undefined;
  const hasPublicSessionHandNo = Object.prototype.hasOwnProperty.call(candidate, "publicSessionHandNo");
  const publicSessionHandNo = hasPublicSessionHandNo
    ? Math.max(1, normalizeNonNegativeInt(candidate.publicSessionHandNo, 1))
    : undefined;
  const publicLastHandSummary = sanitizeOkeyPrototypeLastHandSummary(candidate.publicLastHandSummary);
  ...
  return {
    ...
    publicSeatOpenModes,
    publicRackState: publicRackState ?? undefined,
    publicWallTiles: publicWallTiles ?? undefined,
    publicDiscardPile: publicDiscardPile ?? undefined,
    publicWinnerSeat,
    publicHandDrawn,
    publicHandCompleted,
    publicSeatPenaltyTotals: publicSeatPenaltyTotals ?? undefined,
    publicSeatHandWins: publicSeatHandWins ?? undefined,
    publicDrawHandCount,
    publicSessionHandNo,
...
function getOkeyPrototypeTableUpdatedAt(
  ...
  const explicitUpdatedAt = Number.isFinite(table.updatedAt) ? Number(table.updatedAt) : 0;
  if (explicitUpdatedAt > 0) return explicitUpdatedAt;
  const createdAt = Number.isFinite(table.createdAt) ? Number(table.createdAt) : 0;
  const startedAt = Number.isFinite(table.startedAt) ? Number(table.startedAt) : 0;
  const lastJoinedAt = OKEY_PROTOTYPE_SEATS.reduce((max, seatNo) => {
    const joinedAt = Number(table.seats[seatNo]?.joinedAt ?? 0);
    return Number.isFinite(joinedAt) ? Math.max(max, joinedAt) : max;
  }, 0);
  return Math.max(createdAt, startedAt, lastJoinedAt);
}

function isOkeyPrototypeBotUserIdValue(userId: string) {
  const safe = sanitizeGuestId(userId);
  return safe.startsWith(OKEY_PROTOTYPE_BOT_USER_ID_PREFIX);
}

function isOkeyPrototypeLocalBotTableId(tableId: string) {
  const safeTableId = sanitizeOkeyPrototypeTableId(tableId);
  return safeTableId.startsWith(OKEY_PROTOTYPE_LOCAL_BOT_TABLE_ID_PREFIX);
}

function getOkeyPrototypeOccupiedSeatNosFromSeats(
  seats: Partial<Record<OkeyPrototypeSeatNo, OkeyPrototypeLobbySeatState>>,
) {
  return OKEY_PROTOTYPE_SEATS.filter((seatNo) => Boolean(seats[seatNo]));
}

function normalizeOkeyPrototypeLobbySeat(raw: unknown): OkeyPrototypeLobbySeatState | null {
  if (!raw || typeof raw !== "object") return null;
  const candidate = raw as Partial<OkeyPrototypeLobbySeatState>;
  const userId = sanitizeGuestId(typeof candidate.userId === "string" ? candidate.userId : "");
  const sessionId = sanitizeGuestId(typeof candidate.sessionId === "string" ? candidate.sessionId : "");
  const displayName = sanitizeOkeyPrototypeSeatName(typeof candidate.displayName === "string" ? candidate.displayName : "");
  if (!userId || !sessionId || !displayName) return null;
  ...
  return {
    ...
    points: normalizeNonNegativeInt(candidate.points, 1500),
    stats: normalizeStats(candidate.stats),
    joinedAt: Number.isFinite(candidate.joinedAt) ? Number(candidate.joinedAt) : Date.now(),
  };
}

function normalizeOkeyPrototypeLobbyTable(raw: unknown, roomIdFallback: string, index: number): OkeyPrototypeLobbyTableState | null {
  if (!raw || typeof raw !== "object") return null;
  const candidate = raw as Partial<OkeyPrototypeLobbyTableState>;
  const roomId = sanitizeLobbyId(typeof candidate.roomId === "string" ? candidate.roomId : "") || roomIdFallback;
  const tableNo = normalizeNonNegativeInt(candidate.tableNo, index + 1);
  if (!roomId || tableNo <= 0) return null;
  const id = sanitizeOkeyPrototypeTableId(candidate.id) || `${roomId}-table-${tableNo}`;
  const seatsRaw = candidate.seats && typeof candidate.seats === "object"
    ? candidate.seats as Record<string, unknown>
    : {};
  const seats: Partial<Record<OkeyPrototypeSeatNo, OkeyPrototypeLobbySeatState>> = {};
  Object.entries(seatsRaw).forEach(([seatNoRaw, seatRaw]) => {
    const seatNoValue = Number.parseInt(seatNoRaw, 10);
    if (!OKEY_PROTOTYPE_SEATS.includes(seatNoValue as OkeyPrototypeSeatNo)) return;
    const seat = normalizeOkeyPrototypeLobbySeat(seatRaw);
    if (!seat) return;
    seats[seatNoValue as OkeyPrototypeSeatNo] = seat;
  });
...
function normalizeOkeyPrototypeTablesByRoom(raw: unknown): Record<string, OkeyPrototypeLobbyTableState[]> {
  if (!raw || typeof raw !== "object") return {};
  const candidate = raw as Record<string, unknown>;
  const result: Record<string, OkeyPrototypeLobbyTableState[]> = {};
  Object.entries(candidate).forEach(([roomIdRaw, tablesRaw]) => {
    const roomId = sanitizeLobbyId(roomIdRaw);
    if (!roomId) return;
    const rows = Array.isArray(tablesRaw) ? tablesRaw : [];
    ...
    const normalizedTables = Array.from(byId.values()).sort((left, right) => left.tableNo - right.tableNo);
    if (normalizedTables.length > 0) {
      result[roomId] = normalizedTables;
    }
  });
  return result;
}
...
function cleanupOkeyPrototypeTablesByRoom(
  ...
  Object.entries(normalizedTablesByRoom).forEach(([roomId, tables]) => {
    ...
    tables.forEach((table) => {
      ...
      const safeOwnerUserId = sanitizeGuestId(table.ownerUserId);
      ...
      const startedChanged = startedAt !== table.startedAt;
      const nextTable: OkeyPrototypeLobbyTableState = {
        ...
        ownerSessionId,
        startedAt,
        updatedAt: tableChanged || ownerChanged || startedChanged ? now : table.updatedAt,
      };
      if (tableChanged || ownerChanged || startedChanged) {
        changed = true;
      }
      nextTables.push(nextTable);
    });
    if (nextTables.length > 0) {
      nextTablesByRoom[roomId] = nextTables.sort((left, right) => left.tableNo - right.tableNo);
  ...
  return {
    tablesByRoom: normalizeOkeyPrototypeTablesByRoom(nextTablesByRoom),
    changed,
  };
}
...
function saveOkeyPrototypeSeatReservationToSession(reservation: OkeyPrototypeSeatReservation | null) {
  ...
  const tableId = sanitizeOkeyPrototypeTableId(reservation.tableId);
  const seatNo = normalizeOkeyPrototypeSeatNo(reservation.seatNo);
  if (!tableId || !seatNo) {
    safeStorageRemoveItem(window.sessionStorage, OKEY_PROTOTYPE_SEAT_RESERVATION_SESSION_KEY);
    return;
  }
  safeStorageSetItem(
    window.sessionStorage,
    OKEY_PROTOTYPE_SEAT_RESERVATION_SESSION_KEY,
    JSON.stringify({
      tableId,
      seatNo,
...
function mergeOkeyPrototypeSeatState(
  ...
  if (!baseSeat && incomingSeat) {
    ...
    // Base tablo incoming tablodan daha yeni ise (base daha recent update),
    // base'in null verisini tercih et - bu stale dolu koltuk geri donusunu onler.
    if (baseTableUpdatedAt > incomingTableUpdatedAt) {
      return null;
    }
...
function mergeOkeyPrototypeTablesByRoom(
  ...
  return normalizeOkeyPrototypeTablesByRoom(merged);
}
...
function Okey101App() {
  ...
  const [okeyPrototypeTablesByRoom, setOkeyPrototypeTablesByRoom] = useState<Record<string, OkeyPrototypeLobbyTableState[]>>(
    () => normalizeOkeyPrototypeTablesByRoom(lobbyState.okeyPrototypeTablesByRoom),
  );
  ...
  const [okeyPrototypeSeatOpeningPairCounts, setOkeyPrototypeSeatOpeningPairCounts] = useState<Record<number, number>>(
    () => ({ 1: 0, 2: 0, 3: 0, 4: 0 }),
  );
...
const [okeyPrototypeMeldDraftTileIds, setOkeyPrototypeMeldDraftTileIds] = useState<string[]>([]);
  ...
  const [okeyPrototypeViewportWidth, setOkeyPrototypeViewportWidth] = useState(() => typeof window !== "undefined" ? window.innerWidth : 375);
  const [okeyPrototypeViewportHeight, setOkeyPrototypeViewportHeight] = useState(() => typeof window !== "undefined" ? window.innerHeight : 700);
  ...
  const okeyPrototypeSingleTurnOpenFinishRef = useRef<{
    seatNo: OkeyPrototypeSeatNo;
    round: number;
    noOtherSeatOpenedBeforeTurn: boolean;
    winnerOpenedWithPairs: boolean;
  } | null>(null);
  ...
  const okeyPrototypeLocalBotTableSketchRow = useMemo<OkeyPrototypeTableSketchRow | null>(() => {
    ...
    return {
      ...
      tableNo: okeyPrototypeLocalBotTable.tableNo,
      active: started,
      started,
      startedAt: okeyPrototypeLocalBotTable.startedAt ?? null,
      seated,
      occupiedSeatNos,
  ...
  const okeyPrototypeJoinedTable = useMemo(() => {
    ...
    if (isOkeyPrototypeLocalBotTableId(okeyPrototypeSeatReservation.tableId)) {
      if (!okeyPrototypeLocalBotTableSketchRow) return null;
  ...
  useEffect(() => {
    if (!okeyPrototypeJoinedTable) {
      ...
      setOkeyPrototypeSeatOpeningPairCounts({ 1: 0, 2: 0, 3: 0, 4: 0 });
      return;
    }
  ...
  }, [okeyPrototypeSeatReservation, okeyPrototypeTurnSeat, okeyPrototypeActiveTurnSeats, mode, okeyPrototypeBotModeEnabled, okeyPrototypeJoinedTable]);
  ...
  useEffect(() => {
    ...
    if (seatMelds.length > 0 && !okeyPrototypeSeatOpenedState[seatNo]) return;
    setOkeyPrototypePendingDiscardPickup(null);
  }, [okeyPrototypePendingDiscardTileId, okeyPrototypeOpenedMeldsBySeat, okeyPrototypeSeatOpenedState, okeyPrototypeSeatNoForRack]);
  useEffect(() => {
    if (!okeyPrototypeSeatReservation || !okeyPrototypeJoinedTable) {
      okeyPrototypeSeatOpenNoticesReadyRef.current = false;
      okeyPrototypePrevSeatOpenedStateRef.current = createDefaultOkeyPrototypeSeatOpenedState();
    ...
    if (!okeyPrototypeSeatOpenNoticesReadyRef.current) {
      ...
      const seededPairCounts: Record<number, number> = { 1: 0, 2: 0, 3: 0, 4: 0 };
      OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
        ...
        if (seatOpenMode === "pair") {
          ...
          if (pairCount > 0) seededPairCounts[seatNo] = pairCount;
          return;
        }
        const seatMelds = okeyPrototypeOpenedMeldsBySeat[seatNo] ?? [];
      ...
      setOkeyPrototypeSeatOpeningPairCounts(seededPairCounts);
    ...
    OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
      ...
      if (seatOpenMode === "pair") {
        const pairCount = okeyPrototypeSeatOpeningPairCounts[seatNo] ?? 0;
    ...
    okeyPrototypeSeatOpeningPairCounts,
  ...
  const okeyPrototypeViewportScale = (() => {
    const wScale = okeyPrototypeViewportWidth / 375;
    const hScale = okeyPrototypeViewportHeight / 667;
    return Math.max(0.6, Math.min(1.15, Math.min(wScale, hScale)));
  })();
  const okeyPrototypeTileScale = Math.max(85, Math.min(130, okeyPrototypeTileScalePct)) / 100 * okeyPrototypeViewportScale;
  ...
  const okeyPrototypeLocalSeatMelds = okeyPrototypeOpenedMeldsBySeat[okeyPrototypeLocalSeatNo] ?? [];
  const okeyPrototypeHasInvalidOpening = okeyPrototypeLocalSeatMelds.length > 0 && !okeyPrototypeLocalSeatOpened;
  const okeyPrototypeCanDiscardTile = okeyPrototypeCanAdvanceTurn
    && okeyPrototypeTurnPhase === "discard"
    && okeyPrototypeTurnHasDrawn
    && okeyPrototypeTurnRackTiles.length > 0
    && !okeyPrototypePendingDiscardTileId
    && !okeyPrototypeHasInvalidOpening;
  ...
  const okeyPrototypeCurrentSeatOpenMode = okeyPrototypeSeatOpenModes[okeyPrototypeTurnSeatNo] ?? "none";
  const okeyPrototypePairProcessTargetSeatNos = useMemo(() => {
    if (!okeyPrototypeCurrentSeatOpened) return [] as OkeyPrototypeSeatNo[];
    if (okeyPrototypeCurrentSeatOpenMode === "pair") {
      return [okeyPrototypeTurnSeatNo];
    }
    if (okeyPrototypeCurrentSeatOpenMode === "seri") {
      return OKEY_PROTOTYPE_SEATS.filter((seatNo) => (okeyPrototypeSeatOpenModes[seatNo] ?? "none") === "pair");
    }
    return [] as OkeyPrototypeSeatNo[];
  }, [
    okeyPrototypeCurrentSeatOpened,
    okeyPrototypeCurrentSeatOpenMode,
    okeyPrototypeSeatOpenModes,
    okeyPrototypeTurnSeatNo,
  ]);
  const okeyPrototypePairProcessTargetSeatNoSet = useMemo(
    () => new Set<OkeyPrototypeSeatNo>(okeyPrototypePairProcessTargetSeatNos),
    [okeyPrototypePairProcessTargetSeatNos],
  );
    ...
    && (
      (
        ...
        && (
          ...
          || (
            okeyPrototypeCurrentSeatOpened
            && (okeyPrototypeSeatOpenModes[okeyPrototypeTurnSeatNo] ?? "none") === "seri"
            && okeyPrototypeMeldDraftValidation.kind === "set"
          )
        )
      )
      || (
      ...
      || (
        okeyPrototypeCurrentSeatOpened
        && (okeyPrototypeSeatOpenModes[okeyPrototypeTurnSeatNo] ?? "none") === "seri"
        && okeyPrototypeRackValidMeldGroups.some((group) => group.kind === "seri" || group.kind === "set")
      )
    );
  ...
  const okeyPrototypePairProcessGroups = useMemo(() => {
    ...
    if (okeyPrototypeCurrentSeatOpenMode !== "pair" && okeyPrototypeCurrentSeatOpenMode !== "seri") {
      return [] as OkeyPrototypeTile[][];
    }
    if (okeyPrototypeCurrentSeatOpenMode === "seri" && okeyPrototypePairProcessTargetSeatNos.length === 0) {
      return [] as OkeyPrototypeTile[][];
    }
  ...
  const okeyPrototypePendingCanProcessByKind = useMemo(() => {
    ...
    if (!okeyPrototypePendingDiscardTileId) return fallback;
    if (!okeyPrototypeCanAdvanceTurn || okeyPrototypeTurnPhase !== "discard" || !okeyPrototypeCurrentSeatOpened) {
      return {
        set: false,
        seri: false,
    ...
    const seriProcessable = canAttachDirectly("seri") || (okeyPrototypeCurrentSeatOpenMode === "seri" && okeyPrototypePairProcessTargetSeatNos.length > 0 && setProcessable);
    return {
      set: setProcessable,
  ...
  const okeyPrototypeCanProcessSeries = (
    okeyPrototypeAttachableCounts.seri > 0
    || okeyPrototypeCanOpenSerialGroupFromRack
    || (okeyPrototypeCurrentSeatOpenMode === "seri"
        && okeyPrototypePairProcessTargetSeatNos.length > 0
        && (okeyPrototypeAttachableCounts.set > 0 || okeyPrototypeCanOpenPairGroupFromRack))
  )
  ...
  const okeyPrototypeAttachValidation = useMemo(() => {
    ...
    const attachValidation = canAttachOkeyPrototypeTileToMeld(
  ...
  const okeyPrototypeBoardHintText = useMemo(() => {
    ...
    if (okeyPrototypeHandCompleted) {
      return okeyPrototypeHandDrawn
        ? "El berabere bitti. Kapali deste bitti; ceza yazildi ve yeni el otomatik baslar."
  ...
  const okeyPrototypeSeatReservationHydratedRef = useRef(false);
  const okeyPrototypeMissingSeatReservationSinceRef = useRef<number | null>(null);
  const okeyPrototypeMissingSeatReservationLastPullAtRef = useRef(0);
  ...
  useEffect(() => {
    const next = normalizeOkeyPrototypeTablesByRoom(lobbyState.okeyPrototypeTablesByRoom);
  ...
  useEffect(() => {
    saveOkeyPrototypeSeatReservationToSession(okeyPrototypeSeatReservation);
  }, [okeyPrototypeSeatReservation]);

  useEffect(() => {
    if (okeyPrototypeSeatReservationHydratedRef.current) return;
  ...
  useEffect(() => {
    if (!okeyPrototypeSeatReservation) {
      okeyPrototypeMissingSeatReservationSinceRef.current = null;
    ...
    if (isOkeyPrototypeLocalBotTableId(okeyPrototypeSeatReservation.tableId)) {
      const localActive = Boolean(
        okeyPrototypeLocalBotTable
        && okeyPrototypeLocalBotTable.id === okeyPrototypeSeatReservation.tableId,
      );
      if (localActive) {
        okeyPrototypeMissingSeatReservationSinceRef.current = null;
        okeyPrototypeMissingSeatReservationLastPullAtRef.current = 0;
    ...
    const now = Date.now();
    const missingSince = okeyPrototypeMissingSeatReservationSinceRef.current ?? now;
    if (!okeyPrototypeMissingSeatReservationSinceRef.current) {
      okeyPrototypeMissingSeatReservationSinceRef.current = now;
    }
    if (now - okeyPrototypeMissingSeatReservationLastPullAtRef.current >= 1500) {
      okeyPrototypeMissingSeatReservationLastPullAtRef.current = now;
      void pullRealtimeViaHttp("okey-seat-reservation-restore");
    }
    if (now - missingSince < OKEY_RECONNECT_NOTICE_GRACE_MS) return;
  ...
  useEffect(() => {
    ...
    if (!okeyPrototypeJoinedTable.started) return;
    const activeSeats = getOkeyLobbyOccupiedSeatNos(okeyPrototypeJoinedTable);
    if (activeSeats.length < 4) return;
    const startedAt = Number.isFinite(okeyPrototypeJoinedTable.startedAt) ? Number(okeyPrototypeJoinedTable.startedAt) : 0;
    if (!startedAt) return;
    const startKey = `${okeyPrototypeJoinedTable.id}:${startedAt}`;
    const previousStartKey = okeyPrototypeLastStartedTableKeyRef.current;
    if (previousStartKey === startKey) return;
    const sameTableContinuation = previousStartKey.startsWith(`${okeyPrototypeJoinedTable.id}:`);
    okeyPrototypeLastStartedTableKeyRef.current = startKey;

    const ownerSeat = activeSeats.find(
  ...
  useEffect(() => {
    ...
    if (liveAction.publicRackState) {
      setOkeyPrototypeRackState({
        1: liveAction.publicRackState[1] ?? [],
        2: liveAction.publicRackState[2] ?? [],
        3: liveAction.publicRackState[3] ?? [],
        4: liveAction.publicRackState[4] ?? [],
      });
    }
    if (liveAction.publicWallTiles) {
    ...
    if (liveAction.publicSeatPenaltyTotals) {
      setOkeyPrototypeSeatPenaltyTotals({
        ...
        2: liveAction.publicSeatPenaltyTotals[2] ?? 0,
        3: liveAction.publicSeatPenaltyTotals[3] ?? 0,
        4: liveAction.publicSeatPenaltyTotals[4] ?? 0,
      });
    }
    if (liveAction.publicSeatHandWins) {
      setOkeyPrototypeSeatHandWins({
        1: liveAction.publicSeatHandWins[1] ?? 0,
        2: liveAction.publicSeatHandWins[2] ?? 0,
        3: liveAction.publicSeatHandWins[3] ?? 0,
        4: liveAction.publicSeatHandWins[4] ?? 0,
      });
    }
    if (typeof liveAction.publicDrawHandCount === "number" && Number.isFinite(liveAction.publicDrawHandCount)) {
      setOkeyPrototypeDrawHandCount(Math.max(0, Math.trunc(liveAction.publicDrawHandCount)));
    }
    if (typeof liveAction.publicSessionHandNo === "number" && Number.isFinite(liveAction.publicSessionHandNo)) {
      setOkeyPrototypeSessionHandNo(Math.max(1, Math.trunc(liveAction.publicSessionHandNo)));
    }
    if (typeof liveAction.publicLastHandSummary === "string") {
      setOkeyPrototypeLastHandSummary(liveAction.publicLastHandSummary);
    }
    if (liveAction.publicWinnerSeat !== undefined) {
      setOkeyPrototypeWinnerSeat(liveAction.publicWinnerSeat ?? null);
    }
    if (liveAction.publicHandDrawn !== undefined) {
      setOkeyPrototypeHandDrawn(Boolean(liveAction.publicHandDrawn));
    }
    if (liveAction.publicHandCompleted === false) {
    ...
    if (shouldQueueAutoNextFromRemote) {
      queueOkeyPrototypeAutoNextHand(
        liveAction.publicAutoNextOutcomeKey ?? "",
        liveAction.publicSessionHandNo,
      );
    }
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const handleResize = () => {
      setOkeyPrototypeViewportWidth(window.innerWidth);
      setOkeyPrototypeViewportHeight(window.innerHeight);
    };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (okeyPrototypeTurnPhase === "draw") {
        ...
        return;
 	    ...
 	    if (!okeyPrototypeCurrentSeatOpened && okeyPrototypeTurnOpeningPoints < OKEY_PROTOTYPE_OPENING_TARGET_POINTS) {
 	        if (!okeyPrototypeHandCompleted && okeyPrototypeAutoSerialOpenGroups.length > 0) {
 	          const botAutoPoints = okeyPrototypeAutoSerialOpenGroups.reduce(
 	            (sum, g) => sum + g.points, 0,
 	          );
 	          if (botAutoPoints >= OKEY_PROTOTYPE_OPENING_TARGET_POINTS) {
 	            openOkeyPrototypeSerialMeld();
 	            return;
 	          }
 	        }
 	        ...
 	        if (botPairCount >= OKEY_PROTOTYPE_PAIR_OPEN_MIN_PAIRS) {
 	          openOkeyPrototypePairs();
 	          return;
 	        }
 	    }
	    if (okeyPrototypeCurrentSeatOpened && !okeyPrototypeHandCompleted) {
	      sortOkeyPrototypeAsPairs();
	      sortOkeyPrototypeAsSeries();
	    }
	    if (okeyPrototypeHandCompleted) return;
      ...
      const dangerousDiscardIds = new Set<string>();
      for (const tile of botRack) {
        if (isOkeyPrototypeJokerTile(tile, okeyPrototypeOkeyTile)) {
          dangerousDiscardIds.add(tile.id);
          continue;
        }
        if (okeyPrototypeOpenedMelds.some((meld) => (
          canAttachOkeyPrototypeTileToMeld(tile, meld, okeyPrototypeOkeyTile).valid
        ))) {
          dangerousDiscardIds.add(tile.id);
        }
  ...
  useEffect(() => {
    ...
    const standings = OKEY_PROTOTYPE_SEATS
      .map((seatNo) => ({
        seatNo,
        name: okeyPrototypeSeatDisplayNames[seatNo] ?? `K${seatNo}`,
        penalty: okeyPrototypeSeatPenaltyTotals[seatNo] ?? 0,
      }))
      .sort((left, right) => (left.penalty - right.penalty) || (left.seatNo - right.seatNo));
    const bestPenalty = standings[0]?.penalty ?? 0;
    const winnerRows = standings.filter((entry) => entry.penalty === bestPenalty);
    const loserRows = standings.filter((entry) => entry.penalty > bestPenalty);
    const winnerText = winnerRows.map((entry) => `${entry.name} (K${entry.seatNo})`).join(", ");
    const loserText = loserRows.length > 0
      ? loserRows.map((entry) => `${entry.name} (K${entry.seatNo})`).join(", ")
      : "Yok";
    const rankingText = standings
      .map((entry) => `${entry.name} (K${entry.seatNo}): ${entry.penalty}`)
      .join(" | ");
    appendOkeyPrototypeAction(
  ...
  const canWriteTableChat = useMemo(() => {
    ...
    if (!member) return true;
    return Boolean(!member.isBlocked && member.permissions.tableChat);
  }, [roomSession, canViewTableChat, mode, currentRoomTable, member, okeyPrototypeSeatReservation]);
  ...
  function updateOkeyPrototypeTablesByRoom(
    ...
    if (selectedGameId !== "tavla") {
      ...
      if (okeyPrototypeSeatReservation) {
        if (isOkeyPrototypeLocalBotTableId(okeyPrototypeSeatReservation.tableId)) {
          ...
          setRoomPickerOpen(false);
          setGamePickerOpen(false);
          setLobbyNotice("Bilgisayar masan zaten acik. Masaya geri donuldu.");
          return;
        }
  ...
  function startLocalOkeyPrototypeBotGame() {
    if (!okeyPrototypeSelectedRoom) {
      setLobbyNotice("Lutfen once bir oda sec.");
      return;
    }
    ...
    const now = Date.now();
    const roomId = okeyPrototypeSelectedRoom.id;
    const localSeatNo = 3 as OkeyPrototypeSeatNo;
    const localTableId = `${OKEY_PROTOTYPE_LOCAL_BOT_TABLE_ID_PREFIX}${appSessionId.slice(-10)}-${now.toString(36)}`;
    const tableMeta = {
      id: localTableId,
      tableNo: 1,
      roomId,
    ...
    const nextSeats: Partial<Record<OkeyPrototypeSeatNo, OkeyPrototypeLobbySeatState>> = {
      [localSeatNo]: getOkeyPrototypeCurrentSeatState(now),
    };
    OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
      if (seatNo === localSeatNo) return;
      nextSeats[seatNo] = createOkeyPrototypeBotSeatState(tableMeta, seatNo, now + seatNo);
    });
    const localTable: OkeyPrototypeLobbyTableState = {
      id: localTableId,
      roomId,
      tableNo: 1,
      ownerUserId: safeUserId,
  ...
  async function reserveOkeyPrototypeSeat(
    ...
    const latestLobbyState = getCurrentLobbyState();
    const activePresenceRows = cleanupPresenceRows(latestLobbyState.presence).presence;
    const activeSessionIds = new Set(
      activePresenceRows
        .map((row) => sanitizeGuestId(row.sessionId))
        .filter((value): value is string => Boolean(value)),
    );
    const activeUserIds = new Set(
      activePresenceRows
        .map((row) => sanitizeGuestId(row.userId))
        .filter((value): value is string => Boolean(value)),
    );
    const isSeatActive = (seat: OkeyPrototypeLobbySeatState | null | undefined) => {
      if (!seat) return false;
      const seatSessionId = sanitizeGuestId(seat.sessionId);
      const seatUserId = sanitizeGuestId(seat.userId);
      if (seatSessionId && activeSessionIds.has(seatSessionId)) return true;
      if (seatUserId && activeUserIds.has(seatUserId)) return true;
      const seatJoinedAt = getOkeyPrototypeSeatJoinedAt(seat);
      const seatLastSeenAt = seatJoinedAt > 0 ? seatJoinedAt : now;
      return now - seatLastSeenAt <= OKEY_PROTOTYPE_INACTIVE_SEAT_PRUNE_MS;
    };
    const roomId = tableRow.roomId;
    let reserveStatus: "ok" | "missing" | "other-table" | "occupied" | "full" = "missing";
    let resolvedTableNo = tableRow.tableNo;
    let startedNow = false;
    let nextOccupiedSeatNos: OkeyPrototypeSeatNo[] = [];
    ...
    updateOkeyPrototypeTablesByRoom((current) => {
      ...
      const seatedAtOtherTable = Object.values(nextState).some((tables) => tables.some((table) => {
        ...
        return OKEY_PROTOTYPE_SEATS.some((seatNo) => {
          ...
          return isSeatActive(occupant);
        });
      }));
      if (seatedAtOtherTable) {
        reserveStatus = "other-table";
        return current;
      }
      ...
      resolvedTableNo = table.tableNo;
      const sanitizedSeats: Partial<Record<OkeyPrototypeSeatNo, OkeyPrototypeLobbySeatState>> = { ...table.seats };
      OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
        const occupant = sanitizedSeats[seatNo];
        if (!occupant) return;
        if (isSeatActive(occupant)) return;
        delete sanitizedSeats[seatNo];
      });
      const occupiedSeatNos = getOkeyPrototypeOccupiedSeatNos({ seats: sanitizedSeats });
      const occupiedSeat = sanitizedSeats[reservedSeat];
      ...
      const nextSeats: Partial<Record<OkeyPrototypeSeatNo, OkeyPrototypeLobbySeatState>> = { ...sanitizedSeats };
      OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
      ...
      nextState[roomId] = roomTables;
      reserveStatus = "ok";
      return nextState;
    });

    if (reserveStatus === "missing") {
      setLobbyNotice("Masa bulunamadi.");
      return;
    }
    if (reserveStatus === "other-table") {
      setLobbyNotice("Ayni anda sadece bir 101 masasinda oturabilirsin.");
      return;
    }
    if (reserveStatus === "occupied") {
    ...
    if (reserveStatus !== "ok") {
      setLobbyNotice("Masaya oturulurken hata olustu.");
      return;
    }
    const finalizedTable = (getLatestOkeyPrototypeTablesByRoom()[roomId] ?? []).find(
      (table) => table.id === tableRow.id,
    ) ?? null;
    if (!finalizedTable) {
    ...
    if (startedNow && nextOccupiedSeatNos.length >= 4) {
      ...
      setOkeyPrototypeDrawHandCount(0);
      setOkeyPrototypeLastHandSummary("");
      const ownerSeat = nextOccupiedSeatNos.find((seatNo) => sanitizeGuestId(finalizedTable.seats[seatNo]?.userId ?? "") === sanitizeGuestId(finalizedTable.ownerUserId ?? ""))
    ...
    appendOkeyPrototypeAction(`${sourceLabel}: Masa ${resolvedTableNo}, Koltuk ${reservedSeat}`);
  }
  ...
  function startOkeyPrototypeGameWithTable(fillMissingWithBots: boolean) {
    ...
    const now = Date.now();
    const latestTablesByRoom = getLatestOkeyPrototypeTablesByRoom();
    const tableEntry = Object.entries(latestTablesByRoom).find(([, tables]) => (
  ...
  function leaveOkeyPrototypeSeat(sourceLabel: string) {
    const reservation = okeyPrototypeSeatReservation;
    if (!reservation) return;
    if (isOkeyPrototypeLocalBotTableId(reservation.tableId)) {
      const localTableNo = okeyPrototypeLocalBotTable?.tableNo ?? 1;
      const localRoomId = okeyPrototypeLocalBotTable?.roomId ?? okeyPrototypeSelectedRoomId;
      appendOkeyPrototypeAction(`${sourceLabel}: Bilgisayar masasi ${localTableNo} kapatildi.`);
      resetOkeyPrototypeSeatSessionState();
      if (localRoomId) {
        setSelectedLobbyId(localRoomId);
        setOkeyPrototypeSelectedRoomId(localRoomId);
        rememberRoomPickerSelection(localRoomId);
      }
      setViewMode("lobby");
      setRoomPickerOpen(false);
      setGamePickerOpen(false);
      setLobbyNotice("Bilgisayar masasi kapatildi.");
      return;
    }
    let leftTableNo = 0;
    let leftRoomId = "";
  ...
  function moveOkeyPrototypeTurnToNextSeat() {
    let seats = okeyPrototypeActiveTurnSeats.length > 0
      ...
      : [okeyPrototypeTurnSeat as OkeyPrototypeSeatNo];
    if (seats.length < 2) {
      const stableSeats = okeyPrototypeLastStableTurnSeatsRef.current;
      if (stableSeats.length >= 2) {
        seats = stableSeats.slice();
      }
    }
  ...
  function buildOkeyPrototypeLiveActionPublicPayload(
    ...
    seatOpenModes: OkeyPrototypeSeatOpenModes,
    rackStateSnapshot?: OkeyPrototypeRackState | null,
    wallTilesSnapshot?: OkeyPrototypeTile[] | null,
    discardPileSnapshot?: OkeyPrototypeDiscardEntry[] | null,
  ) {
    return {
      nextSeatNo: turnSeat,
      nextRound: turnRound,
      ...
      publicRackState: rackStateSnapshot ? {
        ...
        3: (rackStateSnapshot[3] ?? []).slice(),
        4: (rackStateSnapshot[4] ?? []).slice(),
      } as OkeyPrototypeRackState : undefined,
      publicWallTiles: wallTilesSnapshot ? wallTilesSnapshot.slice(0, 106) : undefined,
      publicDiscardPile: discardPileSnapshot ? discardPileSnapshot.slice(0, 40) : undefined,
    };
  }
  ...
  function moveOkeyPrototypeRackTileNearTargetInSlots(
    ...
    setOkeyPrototypeRackSlotsBySeat((current) => {
      ...
      if (insertAt > sourceIndex) {
        for (let index = sourceIndex; index < insertAt; index += 1) {
          nextSlots[index] = nextSlots[index + 1] ?? null;
        }
      } else if (insertAt < sourceIndex) {
        for (let index = sourceIndex; index > insertAt; index -= 1) {
          nextSlots[index] = nextSlots[index - 1] ?? null;
        }
      }
      nextSlots[insertAt] = sourceTileId;
      const changed = nextSlots.some((slotTileId, index) => slotTileId !== normalizedSlots[index]);
      if (!changed) return current;
      setOkeyPrototypeRackState((rackCurrent) => {
        const rack = rackCurrent[seatNo] ?? [];
        const rackTileById = new Map<string, OkeyPrototypeTile>();
        rack.forEach((tile) => {
          rackTileById.set(tile.id, tile);
        });
        const orderedTiles = nextSlots
          .map((tileId) => (tileId ? rackTileById.get(tileId) ?? null : null))
          .filter((tile): tile is OkeyPrototypeTile => Boolean(tile));
        const orderedIds = new Set(orderedTiles.map((tile) => tile.id));
        const trailingTiles = rack.filter((tile) => !orderedIds.has(tile.id));
        const nextRack = [...orderedTiles, ...trailingTiles];
        const rackChanged = nextRack.some((tile, index) => tile.id !== rack[index]?.id);
        if (!rackChanged) return rackCurrent;
        return {
          ...rackCurrent,
          [seatNo]: nextRack,
        };
  ...
  function updateOkeyPrototypeDragPreview(clientX: number, clientY: number) {
    ...
    const el = document.querySelector(".my-okey-drag-cursor-preview") as HTMLElement | null;
    if (el) {
      el.style.left = nextX + 14 + "px";
      el.style.top = nextY - 18 + "px";
    }
  }
  ...
  function handleOkeyPrototypeRackDropToSlot(slotIndex: number, draggedTileIdFromData = "") {
    ...
    const seatNo = okeyPrototypeSeatNoForRack as OkeyPrototypeSeatNo;
    const swappedTargetTileId = (() => {
      const baseSlots = (okeyPrototypeRackSlotsBySeat[seatNo] ?? createEmptyOkeyPrototypeRackSlots()).slice();
      const normalizedSlots = fillOkeyPrototypeRackSlotsFromTiles(baseSlots, okeyPrototypeSeatRackTiles);
      const sourceIndex = normalizedSlots.findIndex((slotTileId) => slotTileId === sourceTileId);
      if (sourceIndex < 0 || sourceIndex === safeSlotIndex) return null;
      return normalizedSlots[safeSlotIndex] ?? null;
    })();
    setOkeyPrototypeRackSlotsBySeat((current) => {
      const baseSlots = current[seatNo] ?? createEmptyOkeyPrototypeRackSlots();
      const normalizedSlots = fillOkeyPrototypeRackSlotsFromTiles(baseSlots, okeyPrototypeSeatRackTiles);
    ...
    if (swappedTargetTileId !== null) {
      setOkeyPrototypeRackState((current) => {
        const rack = [...(current[seatNo] ?? [])];
        const sourceIdx = rack.findIndex((t) => t.id === sourceTileId);
        const targetIdx = rack.findIndex((t) => t.id === swappedTargetTileId);
        if (sourceIdx < 0 || targetIdx < 0) return current;
        const [removed] = rack.splice(sourceIdx, 1);
        const insertAt = targetIdx < sourceIdx ? targetIdx : targetIdx;
        rack.splice(insertAt, 0, removed);
        return { ...current, [seatNo]: rack };
      });
    }
    handleOkeyPrototypeRackDragEnd();
    okeyPrototypeSuppressRackClickRef.current = true;
  ...
  function handleOkeyPrototypeDiscardPocketDrop(targetSeatNo: OkeyPrototypeSeatNo, draggedTileIdFromData = "") {
    ...
    if (targetSeatNo !== okeyPrototypeLocalSeatNo || !okeyPrototypeCanDiscardTile) {
      if (okeyPrototypeHasInvalidOpening) {
        showOkeyPrototypePenaltyBubble("Yanlis el actiniz! 101 ceza yediniz. Geri Topla ile tahtayi temizleyip devam edin.");
      }
      handleOkeyPrototypeRackDragEnd();
      return;
    }
  ...
  function clearOkeyPrototypeTileSelections() {
    const localSeatNo = okeyPrototypeSeatNoForRack as OkeyPrototypeSeatNo;
    const localMelds = okeyPrototypeOpenedMeldsBySeat[localSeatNo] ?? [];
    const seatOpened = okeyPrototypeSeatOpenedState[localSeatNo] ?? false;
    if (localMelds.length > 0 && !seatOpened) {
      const meldTileIds = new Set(localMelds.flat().map((meld) => meld.tiles.map((tile) => tile.id)));
      const meldTiles = meldTileIds.size > 0
        ? localMelds.flatMap((meld) => meld.tiles)
        : [];
      const pickup = okeyPrototypePendingDiscardPickup;
      const hasDiscardPickup = pickup && pickup.seatNo === localSeatNo;
      const discardPickTile = hasDiscardPickup
        ? (meldTiles.find((t) => t.id === pickup.pickedTileId) ?? null)
        : null;
      const regularMeldTiles = discardPickTile
        ? meldTiles.filter((t) => t.id !== discardPickTile.id)
        : meldTiles;
      if (regularMeldTiles.length > 0) {
        setOkeyPrototypeRackState((current) => ({
          ...current,
          [localSeatNo]: [...(current[localSeatNo] ?? []), ...regularMeldTiles],
        }));
      }
      const meldIds = new Set(localMelds.map((meld) => meld.id));
      setOkeyPrototypeOpenedMelds((current) => current.filter((meld) => !meldIds.has(meld.id)));
      setOkeyPrototypeTurnLockedAfterMeld(false);
  ...
  function getOkeyPrototypeJokerReplacementPlan(
    tile: OkeyPrototypeTile,
    meld: OkeyPrototypeMeldEntry,
  ) {
    if (isOkeyPrototypeJokerTile(tile, okeyPrototypeOkeyTile)) {
      return {
        valid: false,
        reason: "Masadaki okeyi almak icin raftan gercek tas secmelisin.",
        jokerTileId: "",
        replacedTiles: meld.tiles.slice(),
      };
    }
    const jokerIndexes = meld.tiles
      .map((meldTile, index) => ({ meldTile, index }))
      .filter(({ meldTile }) => isOkeyPrototypeJokerTile(meldTile, okeyPrototypeOkeyTile));
    ...
    for (const { meldTile, index } of jokerIndexes) {
      if (!meldTile) continue;
      const replacedTiles = meld.tiles.slice();
      replacedTiles[index] = tile;

      if (meld.kind === "set" && meld.tiles.length === 2) {
        const pairMate = meld.tiles[index === 0 ? 1 : 0];
        if (
          pairMate
          && !isOkeyPrototypeJokerTile(pairMate, okeyPrototypeOkeyTile)
          && isOkeyPrototypeExactPairMatch(pairMate, tile)
        ) {
          return {
            valid: true,
            reason: "",
            jokerTileId: meldTile.id,
            replacedTiles,
          };
        }
        continue;
      ...
      const validation = evaluateOkeyPrototypeMeldDraft(replacedTiles, okeyPrototypeOkeyTile);
      if (!validation.valid || !validation.kind) continue;
      const kindMatches = validation.kind === meld.kind || (meld.kind === "seri" && validation.kind === "set");
      if (!kindMatches) continue;
      if (meld.kind === "set") {
        const realColors = new Set<string>();
        for (const rt of replacedTiles) {
          if (!isOkeyPrototypeJokerTile(rt, okeyPrototypeOkeyTile)) {
            realColors.add(rt.color);
          }
        }
        if (realColors.size < 4) {
          continue;
        }
      }
      return {
    ...
    return {
      valid: false,
      reason: "Bu tas secili perdedeki okeyin yerine gecemez.",
      jokerTileId: "",
      replacedTiles: meld.tiles.slice(),
    };
  }
  ...
  function getOkeyPrototypeRoundPenaltyByRules(
    ...
    options?: {
      ...
      doubleForSingleTurnOpenFinish?: boolean;
    },
  ) {
    ...
    if (options?.doubleForSingleTurnOpenFinish) {
      penalty *= 2;
    }
    if (seatOpened) {
      const unopenedJokerCount = seatRack.reduce((count, tile) => {
        const isUnprocessedJoker = tile.kind === "sahte" || isOkeyPrototypeJokerTile(tile, okeyPrototypeOkeyTile);
        return count + (isUnprocessedJoker ? 1 : 0);
      }, 0);
      if (unopenedJokerCount > 0) {
        penalty += unopenedJokerCount * OKEY_PROTOTYPE_OPENED_JOKER_LEFT_PENALTY_POINTS;
      }
    }
    return Math.max(0, Math.trunc(penalty));
  }

  function buildOkeyPrototypeSeatTotalsWithDelta(
    base: Record<OkeyPrototypeSeatNo, number>,
    delta: Record<OkeyPrototypeSeatNo, number>,
  ) {
    return {
      1: Math.trunc((base[1] ?? 0) + (delta[1] ?? 0)),
      2: Math.trunc((base[2] ?? 0) + (delta[2] ?? 0)),
      3: Math.trunc((base[3] ?? 0) + (delta[3] ?? 0)),
      4: Math.trunc((base[4] ?? 0) + (delta[4] ?? 0)),
    } as Record<OkeyPrototypeSeatNo, number>;
  }

  function buildOkeyPrototypeLiveActionHandCompletionPayload(params: {
    winnerSeat: OkeyPrototypeSeatNo | null;
    handDrawn: boolean;
    seatPenaltyTotals: Record<OkeyPrototypeSeatNo, number>;
    seatHandWins: Record<OkeyPrototypeSeatNo, number>;
    drawHandCount: number;
    sessionHandNo: number;
    lastHandSummary: string;
    autoNextOutcomeKey: string;
  }) {
    return {
      publicWinnerSeat: params.winnerSeat,
      publicHandDrawn: params.handDrawn,
      publicHandCompleted: true,
      publicSeatPenaltyTotals: {
        1: params.seatPenaltyTotals[1] ?? 0,
        2: params.seatPenaltyTotals[2] ?? 0,
        3: params.seatPenaltyTotals[3] ?? 0,
        4: params.seatPenaltyTotals[4] ?? 0,
      } as Record<OkeyPrototypeSeatNo, number>,
      publicSeatHandWins: {
        1: params.seatHandWins[1] ?? 0,
        2: params.seatHandWins[2] ?? 0,
        3: params.seatHandWins[3] ?? 0,
        4: params.seatHandWins[4] ?? 0,
      } as Record<OkeyPrototypeSeatNo, number>,
      publicDrawHandCount: Math.max(0, Math.trunc(params.drawHandCount)),
      publicSessionHandNo: Math.max(1, Math.trunc(params.sessionHandNo)),
      publicLastHandSummary: sanitizeOkeyPrototypeLastHandSummary(params.lastHandSummary),
      publicAutoNextOutcomeKey: sanitizeOkeyPrototypeOutcomeKey(params.autoNextOutcomeKey),
    };
  }

  function queueOkeyPrototypeAutoNextHand(outcomeKey: string, handNoOverride?: number) {
    if (!okeyPrototypeSeatReservation) return;
    ...
    const effectiveHandNo = Math.max(1, Math.trunc(handNoOverride ?? okeyPrototypeSessionHandNo));
    const setCompleted = effectiveHandNo >= OKEY_PROTOTYPE_SET_HAND_TARGET;
    const handKey = `${effectiveHandNo}-${outcomeKey}-${setCompleted ? "set" : "hand"}`;
    if (
  ...
  function finishOkeyPrototypeHandAsDraw(
    ...
    discardEntryOverride?: OkeyPrototypeDiscardEntry | null,
    options?: {
      skipPenalties?: boolean;
      skipDrawCounter?: boolean;
      outcomeKey?: string;
    },
  ) {
    const skipPenalties = options?.skipPenalties === true;
    const skipDrawCounter = options?.skipDrawCounter === true;
    ...
    const nextSeatPenaltyTotals = skipPenalties
      ? {
        1: okeyPrototypeSeatPenaltyTotals[1] ?? 0,
        2: okeyPrototypeSeatPenaltyTotals[2] ?? 0,
        3: okeyPrototypeSeatPenaltyTotals[3] ?? 0,
        4: okeyPrototypeSeatPenaltyTotals[4] ?? 0,
      } as Record<OkeyPrototypeSeatNo, number>
      : buildOkeyPrototypeSeatTotalsWithDelta(
        okeyPrototypeSeatPenaltyTotals,
        penaltyDelta,
      );
    if (!skipPenalties) {
    ...
    const nextDrawHandCount = skipDrawCounter
      ...
      : (okeyPrototypeDrawHandCount + 1);
    if (!skipDrawCounter) {
      setOkeyPrototypeDrawHandCount(nextDrawHandCount);
    }
    setOkeyPrototypeLastDrawnTileId("");
    setOkeyPrototypeFinalWallDrawSeat(null);
    setOkeyPrototypePendingDiscardTileId("");
    setOkeyPrototypePendingDiscardPickup(null);
    setOkeyPrototypeDiscardDraftTileId("");
    const penaltyText = skipPenalties
      ? ""
      : OKEY_PROTOTYPE_SEATS
        .map((seat) => `K${seat}: ${penaltyDelta[seat] ?? 0}`)
        .join(" | ");
    const nextHandSummary = skipPenalties
    ...
    const fallbackTile = okeyPrototypeDiscardPile[0]?.tile
      ?? okeyPrototypeOkeyTile
      ?? {
        id: `okey-proto-sahte-fallback-${discardEntryAt}`,
        kind: "sahte" as const,
        color: "sahte" as const,
        value: 0,
      };
    const fallbackDiscardEntry: OkeyPrototypeDiscardEntry = discardEntryOverride ?? {
      id: `okey-proto-draw-discard-${drawDiscardActorSeatNo}-${discardEntryAt}`,
      seatNo: drawDiscardActorSeatNo,
      tile: fallbackTile,
      round: okeyPrototypeTurnRound,
      at: discardEntryAt,
    };
    const discardPileSnapshot = [fallbackDiscardEntry, ...okeyPrototypeDiscardPile]
      .filter((entry, index, list) => list.findIndex((candidate) => candidate.id === entry.id) === index)
      .slice(0, 40);
    const rackStateSnapshot: OkeyPrototypeRackState = {
      1: (rackStateOverride?.[1] ?? okeyPrototypeRackState[1] ?? []).slice(),
      2: (rackStateOverride?.[2] ?? okeyPrototypeRackState[2] ?? []).slice(),
      3: (rackStateOverride?.[3] ?? okeyPrototypeRackState[3] ?? []).slice(),
      4: (rackStateOverride?.[4] ?? okeyPrototypeRackState[4] ?? []).slice(),
    };
    const nextPublicPayload = buildOkeyPrototypeLiveActionPublicPayload(
      drawDiscardActorSeatNo,
      okeyPrototypeTurnRound,
      "draw",
      okeyPrototypeOpenedMelds,
      okeyPrototypeSeatOpenedState,
      okeyPrototypeSeatOpenModes,
      rackStateSnapshot,
      okeyPrototypeWallTiles.slice(),
      discardPileSnapshot,
    );
    const completionPayload = buildOkeyPrototypeLiveActionHandCompletionPayload({
      winnerSeat: null,
      handDrawn: true,
      seatPenaltyTotals: nextSeatPenaltyTotals,
      seatHandWins: okeyPrototypeSeatHandWins,
      drawHandCount: nextDrawHandCount,
      sessionHandNo: okeyPrototypeSessionHandNo,
      lastHandSummary: nextHandSummary,
      autoNextOutcomeKey: options?.outcomeKey || `draw-${nextDrawHandCount}`,
    });
    publishOkeyPrototypeDiscardLiveAction({
      id: `okey-live-discard-hand-end-draw-${discardEntryAt}-${Math.random().toString(36).slice(2, 7)}`,
      kind: "discard",
      actorSeatNo: drawDiscardActorSeatNo,
      discardEntry: fallbackDiscardEntry,
    ...
    appendOkeyPrototypeAction(
      skipPenalties
        ? `${reason} Tur cezasiz iptal edildi.`
        : `${reason} El berabere bitti. Acmayan oyunculara ${OKEY_PROTOTYPE_UNOPENED_PENALTY_POINTS}, `
          + "acanlara elde kalan tas puani yazildi.",
    );
    okeyPrototypeSingleTurnOpenFinishRef.current = null;
    queueOkeyPrototypeAutoNextHand(options?.outcomeKey || `draw-${nextDrawHandCount}`, okeyPrototypeSessionHandNo);
  }
  ...
  function finishOkeyPrototypeHandWithFinalDiscard(
    ...
    options?: {
      winnerJustOpenedThisTurn?: boolean;
      noOtherSeatOpenedBeforeTurn?: boolean;
      winnerOpenedWithPairs?: boolean;
    },
  ) {
    ...
    const winnerOpenedWithPairs = options?.winnerOpenedWithPairs
      ?? ((okeyPrototypeSeatOpenModes[seatNo] ?? "none") === "pair");
    const singleTurnOpenFinish = Boolean(
      options?.winnerJustOpenedThisTurn
      && options?.noOtherSeatOpenedBeforeTurn,
    );
    const winnerRewardPoints = jokerFinish
      ? (
        singleTurnOpenFinish
          ? OKEY_PROTOTYPE_JOKER_SINGLE_TURN_OPEN_WIN_REWARD_POINTS
          : OKEY_PROTOTYPE_JOKER_WIN_REWARD_POINTS
      )
      : OKEY_PROTOTYPE_WIN_REWARD_POINTS;
    const penaltyDelta = createDefaultOkeyPrototypeSeatWinState();
    OKEY_PROTOTYPE_SEATS.forEach((seat) => {
      ...
      penaltyDelta[seat] = getOkeyPrototypeRoundPenaltyByRules(
        ...
        seatRack,
        {
          ...
          doubleForSingleTurnOpenFinish: singleTurnOpenFinish,
        },
      );
    });
    penaltyDelta[seatNo] = (penaltyDelta[seatNo] ?? 0) + winnerRewardPoints;
    const nextSeatPenaltyTotals = buildOkeyPrototypeSeatTotalsWithDelta(
      okeyPrototypeSeatPenaltyTotals,
      penaltyDelta,
    );
    setOkeyPrototypeSeatPenaltyTotals(nextSeatPenaltyTotals);
    ...
    const nextSeatHandWins = {
      ...okeyPrototypeSeatHandWins,
      [seatNo]: (okeyPrototypeSeatHandWins[seatNo] ?? 0) + 1,
    } as Record<OkeyPrototypeSeatNo, number>;
    setOkeyPrototypeSeatHandWins(nextSeatHandWins);
    const penaltyText = OKEY_PROTOTYPE_SEATS
      .filter((seat) => seat !== seatNo)
      .map((seat) => `K${seat}: ${penaltyDelta[seat] ?? 0}`)
      .join(" | ");
    const nextHandSummary = `El ${okeyPrototypeSessionHandNo}: Koltuk ${seatNo} kazandi. Ceza: ${penaltyText}`;
    setOkeyPrototypeLastHandSummary(nextHandSummary);
    const rackStateSnapshot: OkeyPrototypeRackState = {
      1: (okeyPrototypeRackState[1] ?? []).slice(),
      2: (okeyPrototypeRackState[2] ?? []).slice(),
      3: (okeyPrototypeRackState[3] ?? []).slice(),
      4: (okeyPrototypeRackState[4] ?? []).slice(),
    };
    rackStateSnapshot[seatNo] = nextRack.slice();
    const discardPileSnapshot = [discardEntry, ...okeyPrototypeDiscardPile].slice(0, 40);
    const nextPublicPayload = buildOkeyPrototypeLiveActionPublicPayload(
      seatNo,
      okeyPrototypeTurnRound,
      "draw",
    ...
    const completionPayload = buildOkeyPrototypeLiveActionHandCompletionPayload({
      ...
      handDrawn: false,
      seatPenaltyTotals: nextSeatPenaltyTotals,
      seatHandWins: nextSeatHandWins,
      drawHandCount: okeyPrototypeDrawHandCount,
      sessionHandNo: okeyPrototypeSessionHandNo,
      lastHandSummary: nextHandSummary,
      autoNextOutcomeKey: `win-${seatNo}`,
    });
    publishOkeyPrototypeDiscardLiveAction({
      id: `okey-live-discard-hand-end-win-${discardEntry.id}`,
      kind: "discard",
      actorSeatNo: seatNo,
      discardEntry,
      nextSeatNo: nextPublicPayload.nextSeatNo,
    ...
    if (singleTurnOpenFinish) {
      appendOkeyPrototypeAction("Tek elde acip bitis: acmamis rakip cezalarina x2 uygulandi.");
    }
    appendOkeyPrototypeAction(`Kazanan puani: K${seatNo} ${winnerRewardPoints}.`);
    appendOkeyPrototypeAction("Yeni el otomatik olarak baslayacak.");
    okeyPrototypeSingleTurnOpenFinishRef.current = null;
    queueOkeyPrototypeAutoNextHand(`win-${seatNo}`, okeyPrototypeSessionHandNo);
  }
  ...
  function openOkeyPrototypeMeldGroups(
    ...
    if (validOpeningNow) {
      okeyPrototypeSingleTurnOpenFinishRef.current = {
        seatNo,
        round: okeyPrototypeTurnRound,
        noOtherSeatOpenedBeforeTurn: !anySeatOpenedBefore,
        winnerOpenedWithPairs: false,
      };
    }
    if (validOpeningNow && !anySeatOpenedBefore && consumedPendingDiscardTile && okeyPrototypePendingDiscardPickup) {
    ...
    if (validOpeningNow && finalDiscardTile) {
  ...
  function openOkeyPrototypePairs() {
    ...
    const pendingDiscardTileId = (
      ...
      ? okeyPrototypePendingDiscardTileId
      : "";
    if (okeyPrototypePendingDiscardTileId && !pendingDiscardTileId) {
      setOkeyPrototypePendingDiscardTileId("");
      setOkeyPrototypePendingDiscardPickup(null);
    }
    const slotOrder = seatNo === okeyPrototypeSeatNoForRack ? okeyPrototypeSeatRackSlots : null;
    ...
    const openablePairs = pairPlan.groups.slice();
    if (pairCount < OKEY_PROTOTYPE_PAIR_OPEN_MIN_PAIRS) {
      appendOkeyPrototypeAction(
        `Uyari: Cift acma puani yetmiyor (${pairCount}/${OKEY_PROTOTYPE_PAIR_OPEN_MIN_PAIRS} cift). `
        + `Geri Topla ile tahtayi temizleyip 101 ceza yersin.`,
      );
    }
    ...
    const consumedPendingDiscardTile = pendingDiscardTileId
      ? openedPairTileIds.has(pendingDiscardTileId)
      : false;
    if (pendingDiscardTileId && !openedPairTileIds.has(pendingDiscardTileId)) {
      const pendingTile = okeyPrototypeTurnRackTiles.find((tile) => tile.id === pendingDiscardTileId) ?? null;
      appendOkeyPrototypeAction(
    ...
    if (nextTileCountAfterOpen === 0) {
      ...
      if (pairsForEntries.length < OKEY_PROTOTYPE_PAIR_OPEN_MIN_PAIRS) {
        appendOkeyPrototypeAction("Uyari: Son tas atildiginda 5 cift kalmadi.");
      }
    }
    const now = Date.now();
    ...
    if (validPairOpening) {
      ...
      setOkeyPrototypeSeatOpeningPairCounts((current) => ({
        ...current,
        [seatNo]: pairCount,
      }));
    ...
    if (validPairOpening) {
      okeyPrototypeSingleTurnOpenFinishRef.current = {
        ...
        round: okeyPrototypeTurnRound,
        noOtherSeatOpenedBeforeTurn: !anySeatOpenedBefore,
        winnerOpenedWithPairs: true,
      };
      const activeSeatNos = okeyPrototypeActiveTurnSeats.length > 0
        ? okeyPrototypeActiveTurnSeats
        : OKEY_PROTOTYPE_SEATS;
      const allSeatsPairOpened = activeSeatNos.length === 4
        && activeSeatNos.every((candidateSeatNo) => {
          if (candidateSeatNo === seatNo) return true;
          return Boolean(okeyPrototypeSeatOpenedState[candidateSeatNo])
            && (okeyPrototypeSeatOpenModes[candidateSeatNo] ?? "none") === "pair";
        });
      if (allSeatsPairOpened) {
        finishOkeyPrototypeHandAsDraw(
          "Dort oyuncu cift actigi icin:",
          undefined,
          null,
          {
            skipPenalties: true,
            skipDrawCounter: true,
            outcomeKey: `pair-cancel-${Date.now()}`,
          },
        );
        return;
      }
  ...
  function attachOkeyPrototypeTileToTargetMeld(
    ...
    if (replacementPlan.valid) {
      ...
      setOkeyPrototypeRackState((current) => ({
        ...current,
        [seatNo]: (current[seatNo] ?? []).map((entry) => (entry.id === replacementTile.id ? jokerTile : entry)),
      }));
      setOkeyPrototypeOpenedMelds((current) => current.map((meld) => {
        if (meld.id !== targetMeld.id) return meld;
        return {
          ...meld,
          tiles: replacementPlan.replacedTiles.slice(),
        };
      }));
      if (okeyPrototypePendingDiscardTileId && replacementTile.id === okeyPrototypePendingDiscardTileId) {
        setOkeyPrototypePendingDiscardTileId("");
      }
  ...
  function drawOkeyPrototypeTile(
    ...
    setOkeyPrototypeRackState((current) => ({
      ...
      [seatNo]: (() => {
        ...
        if (placement || okeyPrototypeAutoSortMode === "off") return nextRack;
  ...
  function discardOkeyPrototypeTileWithForced(
    ...
    let normalizedForcedTileId = forcedTileId;
    if (okeyPrototypePendingDiscardTileId && !normalizedForcedTileId) {
      normalizedForcedTileId = okeyPrototypePendingDiscardTileId;
    }
    let droppedTile = currentRack[currentRack.length - 1];
    let nextRack = currentRack.slice(0, -1);
    const selectedTileId = normalizedForcedTileId || okeyPrototypeDiscardDraftTileId;
    if (selectedTileId) {
    ...
    if (nextRack.length === 0) {
      const singleTurnOpenFinishContext = (
        okeyPrototypeSingleTurnOpenFinishRef.current
        && okeyPrototypeSingleTurnOpenFinishRef.current.seatNo === seatNo
        && okeyPrototypeSingleTurnOpenFinishRef.current.round === okeyPrototypeTurnRound
      )
        ? okeyPrototypeSingleTurnOpenFinishRef.current
        : null;
      finishOkeyPrototypeHandWithFinalDiscard(
        ...
        "Tas atma",
        nextRack,
        singleTurnOpenFinishContext
          ? {
            winnerJustOpenedThisTurn: true,
            noOtherSeatOpenedBeforeTurn: singleTurnOpenFinishContext.noOtherSeatOpenedBeforeTurn,
            winnerOpenedWithPairs: singleTurnOpenFinishContext.winnerOpenedWithPairs,
          }
          : undefined,
      );
      return;
    ...
    const nextRackStateSnapshot: OkeyPrototypeRackState = {
      1: (okeyPrototypeRackState[1] ?? []).slice(),
      2: (okeyPrototypeRackState[2] ?? []).slice(),
      3: (okeyPrototypeRackState[3] ?? []).slice(),
      4: (okeyPrototypeRackState[4] ?? []).slice(),
    };
    nextRackStateSnapshot[seatNo] = nextRack.slice();
    const nextDiscardPileSnapshot = [discardEntry, ...okeyPrototypeDiscardPile].slice(0, 40);
    ...
    const nextPublicPayload = buildOkeyPrototypeLiveActionPublicPayload(
      ...
      okeyPrototypeSeatOpenedState,
      okeyPrototypeSeatOpenModes,
      nextRackStateSnapshot,
      okeyPrototypeWallTiles.slice(),
      nextDiscardPileSnapshot,
    );
    publishOkeyPrototypeDiscardLiveAction({
  ...
  function processOkeyPrototypeAttachments(kind: OkeyPrototypeMeldKind, sourceLabel: string) {
    ...
    const actingSeatOpenMode = okeyPrototypeSeatOpenModes[seatNo] ?? "none";
    const pairProcessEntrySeatNo = actingSeatOpenMode === "pair"
      ? seatNo
      : (okeyPrototypePairProcessTargetSeatNos[0] ?? seatNo);
    if (kind === "set" && actingSeatOpenMode === "seri" && okeyPrototypePairProcessTargetSeatNos.length === 0) {
      appendOkeyPrototypeAction(`${sourceLabel}: Masada cift acmis oyuncu olmadigi icin cift isle yapilamaz.`);
      return;
    }
    ...
    const collectProtectedSerialTileIds = () => {
      ...
      okeyPrototypeRackValidMeldGroups.forEach((group) => {
        if (group.kind !== "seri" || group.tiles.length < 3) return;
        if (workingRack.length - group.tiles.length < 1) return;
        if (!group.tiles.every((tile) => workingRackIds.has(tile.id))) return;
        if (!areOkeyPrototypeTilesAdjacentInSlotOrder(group.tiles, okeyPrototypeSeatRackSlots)) return;
        group.tiles.forEach((tile) => protectedIds.add(tile.id));
      });
      return protectedIds;
    };
    ...
    while (guard < 80) {
      guard += 1;
      let changed = false;
      const protectedSerialTileIds = collectProtectedSerialTileIds();
      for (let tileIndex = 0; tileIndex < workingRack.length; tileIndex += 1) {
        ...
        if (replacementTarget) {
          const replacementPlan = getOkeyPrototypeJokerReplacementPlan(tile, replacementTarget);
          const jokerIndex = replacementTarget.tiles.findIndex((entry) => entry.id === replacementPlan.jokerTileId);
    ...
    if ((kind === "set" || (kind === "seri" && okeyPrototypePairProcessTargetSeatNos.length > 0)) && workingRack.length >= 2) {
      const hasPendingDiscardConstraint = Boolean(
        okeyPrototypePendingDiscardPickup
        && okeyPrototypePendingDiscardPickup.seatNo === seatNo
    ...
    if (requiredPendingTileId && !attachedTileIds.has(requiredPendingTileId)) {
      appendOkeyPrototypeAction(
        `${sourceLabel}: Soldan alinan tasi once islemelisin`
        + `${requiredPendingTile ? ` (${formatOkeyPrototypeTile(requiredPendingTile)})` : ""}.`,
      );
      return;
    }
    setOkeyPrototypeRackState((current) => ({
  ...
  function openOkeyPrototypeSerialMeld() {
    ...
    const seatNo = okeyPrototypeTurnSeat as OkeyPrototypeSeatNo;
    const seatOpenMode = okeyPrototypeSeatOpenModes[seatNo] ?? "none";
    if (okeyPrototypeCurrentSeatOpened && seatOpenMode === "pair") {
      appendOkeyPrototypeAction("Cift acmis oyuncu Seri Ac kullanamaz. Seri eklemek icin Seri Isle kullan.");
      return;
    }
    const canOpenFromDraft = (
      okeyPrototypeMeldDraftTiles.length >= 3
      && okeyPrototypeMeldDraftValidation.valid
      && (
        okeyPrototypeMeldDraftValidation.kind === "seri"
    ...
    const autoGroups = !okeyPrototypeCurrentSeatOpened
      ...
      : (
        seatOpenMode === "seri"
          ? okeyPrototypeRackValidMeldGroups
            .filter((group) => group.kind === "seri" || group.kind === "set")
            .map((group) => ({
              tiles: group.tiles,
              kind: group.kind,
            }))
    ...
    if (autoGroups.length === 0) {
      if (okeyPrototypeCurrentSeatOpened && seatOpenMode === "seri") {
        if (okeyPrototypeMeldDraftTiles.length < 3) {
          appendOkeyPrototypeAction("Ilave per acmak icin en az 3 tas sec veya rafinda gecerli per olustur.");
        } else if (!okeyPrototypeMeldDraftValidation.valid) {
          appendOkeyPrototypeAction(`Ilave per acma gecersiz: ${okeyPrototypeMeldDraftValidation.reason}`);
        } else {
          appendOkeyPrototypeAction("Secili grup gecerli bir per degil.");
        }
        return;
      }
      if (okeyPrototypeMeldDraftTiles.length < 3) {
    ...
    openOkeyPrototypeMeldGroups(autoGroups, okeyPrototypeCurrentSeatOpened ? "Seri acma (ilave)" : "Seri acma (otomatik)");
  }
  ...
  function goToLobbyFromTableView() {
    ...
    if (okeyPrototypeSeatReservation) {
      if (isOkeyPrototypeLocalBotTableId(okeyPrototypeSeatReservation.tableId)) {
        setLobbyNotice("Lobiye gecildi. Bilgisayar oyununa Masaya Don ile geri donebilirsin.");
        return;
      }
  ...
  function returnToActiveTableView() {
    ...
    if (okeyPrototypeSeatReservation) {
      if (isOkeyPrototypeLocalBotTableId(okeyPrototypeSeatReservation.tableId)) {
        ...
        setOkeyPrototypeSelectedRoomId(okeyPrototypeLocalBotTable.roomId);
        setOkeyPrototypeSelectedTableId(okeyPrototypeLocalBotTable.id);
        setViewMode("table");
        setRoomPickerOpen(false);
        setGamePickerOpen(false);
        setLobbyNotice("Bilgisayar masasına geri donuldu.");
        return;
      }
      const reservedTable = Object.values(okeyPrototypeTablesByRoom)
        .flat()
  ...
  useEffect(() => {
    const onBeforeUnload = () => {
      ...
      Object.entries(latestOkeyTablesByRoom).forEach(([roomId, tables]) => {
        const nextTables: OkeyPrototypeLobbyTableState[] = [];
        let roomOkeyTablesChanged = false;
        tables.forEach((table) => {
          const nextSeats: Partial<Record<OkeyPrototypeSeatNo, OkeyPrototypeLobbySeatState>> = { ...table.seats };
          let removedMine = false;
          OKEY_PROTOTYPE_SEATS.forEach((seatNo) => {
    ...
    <main className="my-shell" style={designCssVars}>
      ...
      {viewMode === "lobby" ? (
                  ...
                  <section className="my-game-coming-table-sketch">
                    ...
                    <div className="my-game-coming-table-seat-actions">
                      ...
                      <div className="my-game-coming-prototype-turn">
                        ...
                        >
                          {okeyPrototypeHandDrawn
                            ? "El berabere bitti: Kapali deste bitti, ceza puanlari yazildi."
                            : okeyPrototypeHandCompleted
                            ? `El tamamlandi: Koltuk ${okeyPrototypeWinnerSeat} kazandi.`
                            : okeyPrototypeCanAdvanceTurn
                        ...
                        {okeyPrototypeHandCompleted ? (
                          <p className="my-game-coming-prototype-turn-complete" role="status" aria-live="polite" aria-atomic="true">
                            {okeyPrototypeHandDrawn
                              ? "El berabere bitti. Kisa sure icinde yeni el otomatik baslar."
                      ...
                      <div className="my-game-coming-prototype-board">
                        ...
                        <div className="my-game-coming-prototype-board-status" role="status" aria-live="polite" aria-atomic="true">
                          ...
                          <button
                            ...
                            disabled={!okeyPrototypeHasInvalidOpening}
                            aria-describedby="okey-prototype-turn-status"
                          >
                            Secimi Temizle
                          </button>
                        ...
                        <div className="my-game-coming-prototype-rack-tiles" aria-label={`Koltuk ${okeyPrototypeSeatNoForRack} tas dizilimi`}>
                          {okeyPrototypeSeatRackTiles.map((tile) => (
                            ...
                            <button
                              ...
                              onDragOver={(event) => {
                                if (okeyPrototypeRackDragTileId === tile.id) return;
                                event.preventDefault();
                                if (event.dataTransfer) event.dataTransfer.dropEffect = "move";
                              }}
                              onDrop={(event) => {
        ...
        <section className={`my-room-view my-room-view-topless ${roomSession ? "in-room" : "local-mode"} ${!isTavlaSelectedGame ? "my-okey-room-view" : ""}`}>
          ...
          {!isTavlaSelectedGame ? (
            <section
              className={`my-game-coming-room-shell my-okey-room-shell${okeyPrototypeSeatReservation ? " okey101-landscape-v2" : ""}`}
              data-table-active={okeyPrototypeSeatReservation ? "true" : "false"}
              style={{ "--okey-proto-tile-scale": String(okeyPrototypeTileScale) } as CSSProperties}
            >
              <div className="my-game-coming-room-head">
                ...
                {!okeyPrototypeSeatReservation || !okeyPrototypeJoinedTable ? (
                  ...
                  <>
                    ...
                    <div className="my-game-coming-prototype-board-grid" aria-label="101 okey oyun masasi">
                      <div className="okey101-landscape-badge" aria-hidden="true">101</div>
                      <section className="my-game-coming-prototype-board-center" aria-label="Masa merkezi">
                        <div className="my-okey-center-main-layout">
                          <div className="my-okey-center-surface">
                            {([1, 2, 3, 4] as OkeyPrototypeSeatNo[]).map((displayPosition) => {
                              ...
                              const seatName = okeyPrototypeSeatNameByDisplayPosition[displayPosition] || `K${seatNo}`;
                               ...
                               const seatPairMelds = seatMelds.filter(
                                 (meld) => meld.kind === "set" && meld.tiles.length === 2,
                               );
                              return (
                                <section
                                ...
                                >
                                  ...
                                  <div className="my-okey-center-seat-melds">
                                    {seatSerialMelds.length === 0 && seatPairMelds.length === 0 ? (
                                      ...
                                      <>
                                        ...
                                        {seatPairMelds.map((meld) => (
                                          <div
                                            key={meld.id}
                                            className="my-okey-center-seat-meld kind-cift"
                                            data-okey-meld-id={meld.id}
                                          >
                                            {meld.tiles.map((tile) => {
                                              const tileIsJoker = isOkeyPrototypeJokerTile(tile, okeyPrototypeOkeyTile);
                          ...
                          <aside className="my-okey-center-side-lanes" aria-label="Cift acma sutunlari">
                            <div className="my-okey-center-middle-controls my-okey-center-middle-controls-desktop" aria-label="Orta kontrol alani">
                              <div className="my-okey-indicator-card">
                                <span>Gösterge</span>
                                <div className="my-okey-indicator-card-tile-wrap">
                                  {okeyPrototypeIndicatorTile ? (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  className="my-action-btn soft my-okey-btn-geri-topla"
                                  onClick={clearOkeyPrototypeTileSelections}
                                  disabled={!okeyPrototypeHasInvalidOpening}
                                >
                                  Geri Topla
                      ...
                      <aside className="my-okey-landscape-deck-lane" aria-label="Gosterge ve kapali deste">
                        <div className="my-okey-indicator-card">
                          <span>Gösterge</span>
                          <div className="my-okey-indicator-card-tile-wrap">
                            {okeyPrototypeIndicatorTile ? (
                        ...
                        <div className="okey101-landscape-open-score" aria-live="polite" title={okeyPrototypeRackScoreTitle}>
                          <strong>{okeyPrototypeRackScoreValue}</strong>
                        </div>
                      </aside>
                          ...
                          >
                            ...
                            <header>
                              ...
                              {seatDrawTileIsAnimating && okeyPrototypeDeckDrawTile ? (
                                <span
                                  className={`my-game-coming-prototype-rack-tile my-okey-seat-draw-arrive pos-${displaySeatPosition} okey-flipped`}
                                  aria-hidden="true"
                                />
                              ) : null}
                            </header>
                ...
                <>
                      ...
                      <div className="my-game-coming-prototype-rack-actions my-okey-rack-top-actions">
                        <div className="my-okey-open-actions-col">
                          <button
                            type="button"
                            className={`my-action-btn soft my-okey-btn-cift-ac ${okeyPrototypeCanUsePairOpenAction ? "suggested" : ""}`}
                            onClick={openOkeyPrototypePairs}
                            disabled={!okeyPrototypeCanUsePairOpenAction}
                          >
                            Çift Aç
                        ...
                        <div className="my-okey-open-actions-col">
                          <button
                            type="button"
                            className={`my-action-btn soft my-okey-btn-seri-ac ${okeyPrototypeCanUseSerialOpenAction ? "suggested" : ""}`}
                        ...
                        <button
                          type="button"
                          className="my-action-btn soft my-okey-btn-taslari-isle"
                          onClick={sortOkeyPrototypeAsSeries}
                          disabled={!okeyPrototypeCanProcessSeries}
                        >
                          Taşları İşle
